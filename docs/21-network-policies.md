...
```yaml
# rendered by repo/charts/tickethub (templates/networkpolicy.yaml; default-deny)
...
```yaml
# rendered by repo/charts/tickethub (templates/networkpolicy.yaml; orders-allow)
apiVersion: networking.k8s.io/v1
kind: NetworkPolicy
metadata:
  name: orders-allow
